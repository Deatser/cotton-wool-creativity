...
import { firebase, whoAmI, call } from './fb.js?v=4789b125';

/* Значение подставляет сборщик, беря его прямо из serve.py: держать одно
   и то же число в двух файлах руками уже подвело. Однажды его подняли
   в сервере и забыли здесь, и в админке неделю висела плашка «сайт
   обновился, нажмите Ctrl+F5», которую перезагрузка убрать не могла. */
...
/** Сайт открыт с моего компьютера или уже на хостинге: сообщения об ошибках
    должны быть разные. Своя копия, а не общая из fb.js: в браузере хозяйки сайта
  ...
  try {
    ...
    if (stale(pong)) {
      ...
      warn = local()
        ...
        : 'Сайт сейчас обновляется. Загляните через минуту, страница ' +
          'обновится сама.';
    }
  } catch (e) {
    warn = local()
...
function validate() {
  ...
  // объяснения читается как поломка админки: хозяйка сайта так и решила,
